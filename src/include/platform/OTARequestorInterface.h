...
public:
    ...
    public:
         ...
         */
        Iterator(const app::Clusters::OtaSoftwareUpdateRequestor::Structs::ProviderLocation::Type * const list,
                 size_t size)
        {
            mList         = list;
            mListSize     = size;
        ...
        bool Next()
        {
            ...
            {
                if (mList[i].GetFabricIndex() != kUndefinedFabricId)
                {
                    mCurrentIndex = i;
    ...
    CHIP_ERROR Add(const app::Clusters::OtaSoftwareUpdateRequestor::Structs::ProviderLocation::Type & providerLocation)
    {
        for (size_t i = 0; i < mMaxSize; i++)
        {
            if (mList[i].GetFabricIndex() == kUndefinedFabricId)
            {
                mList[i].SetFabricIndex(providerLocation.GetFabricIndex());
                mListSize++;
    ...
    CHIP_ERROR Delete(const app::Clusters::OtaSoftwareUpdateRequestor::Structs::ProviderLocation::Type & providerLocation)
    {
        for (size_t i = 0; i < mMaxSize; i++)
        {
            if (mList[i].GetFabricIndex() != kUndefinedFabricId)
            {
                app::Clusters::OtaSoftwareUpdateRequestor::Structs::ProviderLocation::Type pl = mList[i];
                ...
                {
                    mList[i].SetFabricIndex(kUndefinedFabricId);
                    mListSize--;
                    return CHIP_NO_ERROR;
                }
            }
